...
module chiral_constants 
  ...
  real*8 :: nexp_NN
  real*8, parameter :: gA = 1.28899999999999992D+00  !1.29000000000000004D+00  !1.29d0 
  ...
  real*8, parameter :: hA2 = hA*hA
  real*8, parameter :: hA4 = hA2*hA2
  real*8, parameter :: fpi = 9.22000000000000028D+01 !9.24000000000000057D+01 !92.4d0 
  ...
  !real*8, PARAMETER :: lambda_delta = 450d0
...
CONTAINS 
  ...
  subroutine init_chp_constants 
    ...
    mnuc(-1) = proton_mass
    mnuc(0)  = 9.38918267117927371D+02 !0.5d0*(proton_mass+neutron_mass)
    mnuc(+1) = neutron_mass
    ...
    if(abs(lambda_delta - 394.d0) < 1.d-5) then
       nexp_NN = 4.0D0
    else
       nexp_NN = 3.0D0
    end if
    if(iam==0) write(6,*) "NN regulator Exponent: ", nexp_NN
    ...
    if(chiral_delta_flag)then
       ...
       if(delta_chiral_order == NLO) then
    ...
    if(chiral_delta_flag)then
       if(delta_chiral_order == LO) then
          ...
          if(abs(lambda_delta - 500d0) < 1d-5) then
             !write(6,*) "error no 500 MeV LO interactions"
             !stop
             c1s0(-1) = -1.08522470229349993D-01
             c1s0(0)  = -1.08522470229349993D-01
             c1s0(1)  = -1.08522470229349993D-01
             c3s1(-1) = -6.84437111953200006D-02
             c3s1(0)  = -6.84437111953200006D-02
             c3s1(1)  = -6.84437111953200006D-02

          elseif(abs(lambda_delta - 450d0) < 1d-5 ) then
             c1s0(-1) = -1.12926682348719998D-01 ! Ct_1S0
             c1s0(0)  = -1.12926682348719998D-01 ! Ct_1S0
             c1s0(1)  = -1.12926682348719998D-01 ! Ct_1S0
             c3s1(-1) = -8.73398209265599940D-02 ! Ct_3S1
             c3s1(0)  = -8.73398209265599940D-02 ! Ct_3S1
             c3s1(1)  = -8.73398209265599940D-02 ! Ct_3S1
          
          elseif(abs(lambda_delta - 394d0) < 1d-5 ) then
             c1s0(-1) = -1.16797069688160005D-01 ! Ct_1S0
             c1s0(0)  = -1.16797069688160005D-01! Ct_1S0
             c1s0(1)  = -1.16797069688160005D-01 ! Ct_1S0
             c3s1(-1) = -1.03577257368989997D-01 ! Ct_3S1
             c3s1(0)  = -1.03577257368989997D-01 ! Ct_3S1
             c3s1(1)  = -1.03577257368989997D-01! Ct_3S1
          else
             write(6,*) "Error wrong cutoff"
             stop
          end if
       ...
       if(delta_chiral_order == NLO) then
          ...
          if(abs(lambda_delta - 500d0) < 1d-5 ) then
             c1s0(-1) = -3.09617914433389985D-01 ! Ct_1S0pp   call chp_set_CIB_LO_contact(1, -1, -3.09617914433389985D-01) ! Ct_1S0pp
             c1s0(0)  = -3.10443063173440004D-01 ! Ct_1S0np   call chp_set_CIB_LO_contact(2, -1, -1.91012821791280013D-01) ! Ct_3S1pp
             c1s0(1)  = -3.10256027522040012D-01 ! Ct_1S0nn   call chp_set_CIB_LO_contact(1,  0, -3.10443063173440004D-01) ! Ct_1S0np
             c3s1(-1) = -1.91012821791280013D-01 ! Ct_3S1pp   call chp_set_CIB_LO_contact(2,  0, -1.91012821791280013D-01) ! Ct_3S1np
             c3s1(0)  = -1.91012821791280013D-01 ! Ct_3S1np   call chp_set_CIB_LO_contact(1,  1, -3.10256027522040012D-01) ! Ct_1S0nn
             c3s1(1)  = -1.91012821791280013D-01 ! Ct_3S1nn   call chp_set_CIB_LO_contact(2,  1, -1.91012821791280013D-01) ! Ct_3S1nn
                    
          elseif(abs(lambda_delta - 450d0) < 1d-5 ) then
             c1s0(-1) = -3.09893184381559994D-01 ! Ct_1S0pp   call chp_set_CIB_LO_contact(1, -1, -3.09893184381559994D-01) ! Ct_1S0pp 
             c1s0(0)  = -3.10711872064980021D-01 ! Ct_1S0np   call chp_set_CIB_LO_contact(2, -1, -1.97950619113270027D-01) ! Ct_3S1pp 
             c1s0(1)  = -3.10510995610929985D-01 ! Ct_1S0nn   call chp_set_CIB_LO_contact(1,  0, -3.10711872064980021D-01) ! Ct_1S0np 
             c3s1(-1) = -1.97950619113270027D-01 ! Ct_3S1pp   call chp_set_CIB_LO_contact(2,  0, -1.97950619113270027D-01) ! Ct_3S1np 
             c3s1(0)  = -1.97950619113270027D-01 ! Ct_3S1np   call chp_set_CIB_LO_contact(1,  1, -3.10510995610929985D-01) ! Ct_1S0nn 
             c3s1(1)  = -1.97950619113270027D-01 ! Ct_3S1nn   call chp_set_CIB_LO_contact(2,  1, -1.97950619113270027D-01) ! Ct_3S1nn 
          
          elseif(abs(lambda_delta - 394d0) < 1d-5 ) then
             c1s0(-1) = -3.09925223545309991D-01 ! Ct_1S0pp  call chp_set_CIB_LO_contact(1, -1, -3.09925223545309991D-01) ! Ct_1S0pp
             c1s0(0)  = -3.10751704075070023D-01 ! Ct_1S0np  call chp_set_CIB_LO_contact(2, -1, -2.05462289226700012D-01) ! Ct_3S1pp
             c1s0(1)  = -3.10539966723270000D-01 ! Ct_1S0nn  call chp_set_CIB_LO_contact(1,  0, -3.10751704075070023D-01) ! Ct_1S0np
             c3s1(-1) = -2.05462289226700012D-01 ! Ct_3S1pp  call chp_set_CIB_LO_contact(2,  0, -2.05462289226700012D-01) ! Ct_3S1np
             c3s1(0)  = -2.05462289226700012D-01 ! Ct_3S1np  call chp_set_CIB_LO_contact(1,  1, -3.10539966723270000D-01) ! Ct_1S0nn
             c3s1(1)  = -2.05462289226700012D-01 ! Ct_3S1nn  call chp_set_CIB_LO_contact(2,  1, -2.05462289226700012D-01) ! Ct_3S1nn
          else
             write(6,*) "Error"
             stop
          end if
       ...
       if(delta_chiral_order == NNLO) then
          !these values for lambda_delta = 500d0
          if(abs(lambda_delta - 500d0) < 1d-5 ) then
             c1s0(-1) = -3.37302636943159984D-01 ! Ct_1S0pp  call chp_set_CIB_LO_contact(1, -1, -3.37302636943159984D-01) ! Ct_1S0pp
             c1s0(0)  = -3.38320181277209975D-01 ! Ct_1S0np  call chp_set_CIB_LO_contact(2, -1, -2.21721491662439985D-01) ! Ct_3S1pp
             c1s0(1)  = -3.38223065406040024D-01 ! Ct_1S0nn  call chp_set_CIB_LO_contact(1,  0, -3.38320181277209975D-01) ! Ct_1S0np
             c3s1(-1) = -2.21721491662439985D-01 ! Ct_3S1pp  call chp_set_CIB_LO_contact(2,  0, -2.21721491662439985D-01) ! Ct_3S1np
             c3s1(0)  = -2.21721491662439985D-01 ! Ct_3S1np  call chp_set_CIB_LO_contact(1,  1, -3.38223065406040024D-01) ! Ct_1S0nn
             c3s1(1)  = -2.21721491662439985D-01 ! Ct_3S1nn  call chp_set_CIB_LO_contact(2,  1, -2.21721491662439985D-01) ! Ct_3S1nn
          ...
          elseif(abs(lambda_delta - 394d0) < 1d-5 ) then
             ...
             c1s0(0)  = -3.37854531334379982D-01 ! Ct_1S0np call chp_set_CIB_LO_contact(2, -1, -2.36959231730459990D-01) ! Ct_3S1pp
             c1s0(1)  = -3.37722607113270012D-01 ! Ct_1S0nn call chp_set_CIB_LO_contact(1,  0, -3.37854531334379982D-01) ! Ct_1S0np
             c3s1(-1) = -2.36959231730459990D-01 ! Ct_3S1pp call chp_set_CIB_LO_contact(2,  0, -2.36959231730459990D-01) ! Ct_3S1np
             c3s1(0)  = -2.36959231730459990D-01 ! Ct_3S1np call chp_set_CIB_LO_contact(1,  1, -3.37722607113270012D-01) ! Ct_1S0nn
             c3s1(1)  = -2.36959231730459990D-01 ! Ct_3S1nn call chp_set_CIB_LO_contact(2,  1, -2.36959231730459990D-01) ! Ct_3S1nn
          else
    ...
    if(iam==0) then
       write(6,*) "--------"
       write(6,*) "c1s0(-1)", c1s0(-1)
       write(6,*) "c1s0(0)", c1s0(0) 
       write(6,*) "c1s0(1)", c1s0(1) 
       write(6,*) "c3s1(-1)",c3s1(-1)
       write(6,*) "c3s1(0)", c3s1(0) 
       write(6,*) "c3s1(1)", c3s1(1) 
       write(6,*) "--------" 
    END IF
    ...
    if(chiral_delta_flag)then
       ...
       if(delta_chiral_order == NLO) THEN
          ...
          if(abs(lambda_delta - 500d0) < 1d-5 ) then
             cnlo_pw(1) =  2.39537506986395998D+00 ! C_1S0     call chp_set_NLO_contact(1,  2.39537506986395998D+00) ! C_1S0      
             cnlo_pw(2) =  7.27048937924560046D-01 ! C_3P0     call chp_set_NLO_contact(2,  7.27048937924560046D-01) ! C_3P0     
             cnlo_pw(3) =  1.52465155576100003D-02 ! C_1P1     call chp_set_NLO_contact(3,  1.52465155576100003D-02) ! C_1P1     
             cnlo_pw(4) = -9.51416990370669957D-01 ! C_3P1     call chp_set_NLO_contact(4, -9.51416990370669957D-01) ! C_3P1     
             cnlo_pw(5) =  5.39378033769969956D-01 ! C_3S1     call chp_set_NLO_contact(5,  5.39378033769969956D-01) ! C_3S1     
             cnlo_pw(6) =  3.58443125233390003D-01 ! C_3S1-3D1 call chp_set_NLO_contact(6,  3.58443125233390003D-01) ! C_3S1-3D1 
             cnlo_pw(7) = -7.93620896751420002D-01 ! C_3P2     call chp_set_NLO_contact(7, -7.93620896751420002D-01) ! C_3P2     
                    
          elseif(abs(lambda_delta - 450d0) < 1d-5 ) then
             cnlo_pw(1) =  2.39163813513152990D+00 ! C_1S0    call chp_set_NLO_contact(1,  2.39163813513152990D+00) ! C_1S0     
             cnlo_pw(2) =  6.86901585796929948D-01 ! C_3P0    call chp_set_NLO_contact(2,  6.86901585796929948D-01) ! C_3P0     
             cnlo_pw(3) =  4.81266565581999969D-03 ! C_1P1    call chp_set_NLO_contact(3,  4.81266565581999969D-03) ! C_1P1     
             cnlo_pw(4) = -1.00011242015441004D+00 ! C_3P1    call chp_set_NLO_contact(4, -1.00011242015441004D+00) ! C_3P1    
             cnlo_pw(5) =  5.58973150278149977D-01 ! C_3S1    call chp_set_NLO_contact(5,  5.58973150278149977D-01) ! C_3S1     
             cnlo_pw(6) =  3.62094234568600015D-01 ! C_3S1-3D1call chp_set_NLO_contact(6,  3.62094234568600015D-01) ! C_3S1-3D1
             cnlo_pw(7) = -8.08073350643100041D-01 ! C_3P2    call chp_set_NLO_contact(7, -8.08073350643100041D-01) ! C_3P2    
          
          elseif(abs(lambda_delta - 394d0) < 1d-5 ) then
             cnlo_pw(1) =   2.39117370641156990D+00 ! C_1S0     
             cnlo_pw(2) =   6.65731825802269972D-01 ! C_3P0     
             cnlo_pw(3) =  -1.52505123405899980D-02 ! C_1P1     
             cnlo_pw(4) =  -1.04638857865136004D+00 ! C_3P1    
             cnlo_pw(5) =   5.88755038823760013D-01 ! C_3S1     
             cnlo_pw(6) =   3.81315710481519965D-01 ! C_3S1-3D1
             cnlo_pw(7) =  -8.18066377264660027D-01 ! C_3P2    
          else
        ...
        if(delta_chiral_order ==  NNLO)then
           ...
           !these values are for lambda_delta = 500d0
           if(abs(lambda_delta - 500d0) < 1d-5 ) then
              cnlo_pw(1) =  2.48801946794199980D+00 ! C_1S0     call chp_set_NLO_contact(1,  2.48801946794199980D+00) ! C_1S0     
              cnlo_pw(2) =  6.98453619479979970D-01 ! C_3P0     call chp_set_NLO_contact(2,  6.98453619479979970D-01) ! C_3P0    
              cnlo_pw(3) = -1.26513321381799999D-02 ! C_1P1     call chp_set_NLO_contact(3, -1.26513321381799999D-02) ! C_1P1   
              cnlo_pw(4) = -9.37263520317270116D-01 ! C_3P1     call chp_set_NLO_contact(4, -9.37263520317270116D-01) ! C_3P1   
              cnlo_pw(5) =  6.75353391585620022D-01 ! C_3S1     call chp_set_NLO_contact(5,  6.75353391585620022D-01) ! C_3S1    
              cnlo_pw(6) =  3.54479450076249980D-01 ! C_3S1-3D1 call chp_set_NLO_contact(6,  3.54479450076249980D-01) ! C_3S1-3D1
              cnlo_pw(7) = -8.59526137679109903D-01 ! C_3P2     call chp_set_NLO_contact(7, -8.59526137679109903D-01) ! C_3P2   
           ...
           elseif(abs(lambda_delta - 394d0) < 1d-5 ) then
              ...
              cnlo_pw(2) =  6.16935689120689967D-01 ! C_3P0     
              cnlo_pw(3) = -5.26929817733700068D-02 ! C_1P1     
              cnlo_pw(4) = -1.09054771638107995D+00 ! C_3P1    
              cnlo_pw(5) =  7.22641309492930040D-01 ! C_3S1     
              cnlo_pw(6) =  3.75968068144939938D-01 ! C_3S1-3D1
              cnlo_pw(7) = -8.76265471802940010D-01 ! C_3P2    
           else
              write(6,*) "ERROR wrong cutoff"
              stop
           end if
    ...
    !! setup deltas 2PE
    if(chiral_delta_flag) then
       CALL chp_setup_delta_loop_int_data(nof_2PE_loop_int_mu_points, 2.0D0*mpi(2), sfr)
    end if 
...
! XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxXXX
  ...
  !
  !   Functions for Delta-excitations
  !
  ! XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxx
  
  ! The expressions for some of the loop diagrams contains integrals that must be solved numerically
  ! This function sets up the integration meshs used in the numerical integrations
  SUBROUTINE chp_setup_delta_loop_int_data(nof_mu_points, mu_low, mu_high)
    INTEGER, INTENT(IN) :: nof_mu_points
    REAL(8), INTENT(IN) :: mu_high, mu_low
    
    chp_delta_2PE_loop_int_mesh_mu%info = chp_mesh_info(nof_mu_points, mu_low, mu_high)
  ...
  ! q2    : momentum transfer squared
  ! delta : nucleon-delta mass splitting
  ! impi  : determines which mpi2 to use,
  FUNCTION chp_delta_2PE_sfr_loop_D(q2, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: q2
    REAL(8), INTENT(INOUT) :: delta
    INTEGER, INTENT(IN) :: impi
    REAL(8) :: integral, res, q2i
    INTEGER :: i, j, mu_nr
    REAL(8) :: mu, mu2, mut
...
!!$    print*,"x, weights"
    DO j = 1, mu_nr
       ...
       mu2 = mu*mu
       mut = sqrt( mu2 - fourmpi2(impi) )/(2.0D0*delta)
       integral = integral + chp_delta_2PE_loop_int_mesh_mu%pnt(j)%w * ( atan(mut)/ (mu2 + q2) )
!!$       print*,j, mu, chp_delta_2PE_loop_int_mesh_mu%pnt(j)%w, integral
    END DO

!    stop
!!$    print*,"q2: ",q2
!!$    print*,"delta: ",delta
!!$    print*,"res: ", integral/delta
...
! impi  : determines which mpi2 to use,
  ...
  FUNCTION chp_NLO_delta_2PE_single_triangle_Wc_SFR(w, L, S, D, delta) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: S
    REAL(8) , INTENT(IN) :: D
    ...
    !1.d0/(pi2*fpi4)
!!$    print*,"******* Inside Single triangle Wc *******"
!!$    print*,"const(22) = 1.d0/(pi2*fpi4): ",const(22)
!!$    print*, "pi: ",pi, "fpi: ", fpi
!!$    print*,"hA2: ",hA2
!!$    print*,"loop_sigma: ", S
!!$    print*,"loop_w: ", w
!!$    print*,"loop_L", L
!!$    print*,"loop_D: ", D
!!$    print*,"delta_split: ", delta
    
    res = -hA2*const(22)*( (6.0D0*S-w*w)*L + 12.0D0*delta*delta*S*D)/216.0D0
    

  END FUNCTION chp_NLO_delta_2PE_single_triangle_Wc_SFR
  
  FUNCTION chp_NLO_delta_2PE_single_box_Vc_SFR(q2, A, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: q2
    REAL(8) , INTENT(IN) :: A
    REAL(8) , INTENT(IN) :: delta
    INTEGER , INTENT(IN) :: impi
    ...
    REAL(8) :: res
    !const(24) = 1/(pi*fpi4)
    res = -gA2*hA2*const(24)*( 2.0D0*mpi2(impi) + q2 )*( 2.0D0*mpi2(impi) + q2 )*A/(12.0D0*delta)
    
  END FUNCTION chp_NLO_delta_2PE_single_box_Vc_SFR

  FUNCTION chp_NLO_delta_2PE_single_box_Wc_SFR(q2, L, S, D, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: q2
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: S
    REAL(8) , INTENT(IN) :: D
    ...
    REAL(8) :: res
    !const(22) used in this code instead of const(21)
    res = -gA2*hA2*const(22)*( (12.0D0*delta*delta - 20.0D0*mpi2(impi) - 11.0D0*q2 )*L + 6.0D0*S*S*D )/(216.0D0)
    
  END FUNCTION chp_NLO_delta_2PE_single_box_Wc_SFR

  FUNCTION chp_NLO_delta_2PE_single_box_Vt_SFR(w, L, D, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: D
    REAL(8) , INTENT(IN) :: delta
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -gA2*hA2*const(22)*(-2.0D0*L + (w*w - 4.0D0*delta**2)*D )/(48.0D0)
    
  END FUNCTION chp_NLO_delta_2PE_single_box_Vt_SFR


  FUNCTION chp_NLO_delta_2PE_single_box_Vs_SFR(q2,w, L, D, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: q2
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: D
    ...
    REAL(8) :: res
    !optimize?
    res = -q2*chp_NLO_delta_2PE_single_box_Vt_SFR(w,L,D,delta,impi)
    
  END FUNCTION chp_NLO_delta_2PE_single_box_Vs_SFR

  FUNCTION chp_NLO_delta_2PE_single_box_Wt_SFR(w, A, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: A
    REAL(8) , INTENT(IN) :: delta
    INTEGER , INTENT(IN) :: impi
    ...
    res = -gA2*hA2*const(24)*(w*w*A )/(144.0D0*delta)
    
  END FUNCTION chp_NLO_delta_2PE_single_box_Wt_SFR

  FUNCTION chp_NLO_delta_2PE_single_box_Ws_SFR(q2, w, A, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: q2
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: A
    REAL(8) , INTENT(IN) :: delta
    ...
    REAL(8) :: res
    !optimize?
    res = -q2*chp_NLO_delta_2PE_single_box_Wt_SFR(w,A,delta,impi)
    
  END FUNCTION chp_NLO_delta_2PE_single_box_Ws_SFR

  
  FUNCTION chp_NLO_delta_2PE_double_box_Vc_SFR(L, S, D, H, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: S
    REAL(8) , INTENT(IN) :: D
    REAL(8) , INTENT(IN) :: H
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -hA4*const(22)*(-4.0D0*delta*delta*L + S*(H + (S + 8.0D0*delta*delta)*D))/(27.0D0)
    
  END FUNCTION chp_NLO_delta_2PE_double_box_Vc_SFR

  FUNCTION chp_NLO_delta_2PE_double_box_Wc_SFR(w, L, S, D, H, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: S
    REAL(8) , INTENT(IN) :: D
    ...
    REAL(8) :: res
    !const(22) used instead of (21)
    res = -hA4*const(22)*((12.0D0*S-w*w)*L + 3.0D0*S*(H + (8.0*delta*delta-S)*D))/(486.0D0)
    
  END FUNCTION chp_NLO_delta_2PE_double_box_Wc_SFR

  FUNCTION chp_NLO_delta_2PE_double_box_Vt_SFR(w, L, D, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: D
    REAL(8) , INTENT(IN) :: delta
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -hA4*const(22)*(6.0D0*L + (12.0D0*delta*delta - w*w)*D)/(216.0D0)
    
  END FUNCTION chp_NLO_delta_2PE_double_box_Vt_SFR

  FUNCTION chp_NLO_delta_2PE_double_box_Vs_SFR(q2, w, L, D, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: q2
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: D
    ...
    REAL(8) :: res
    !optimize
    res = -q2*chp_NLO_delta_2PE_double_box_Vt_SFR(w, L, D, delta, impi)
    
  END FUNCTION chp_NLO_delta_2PE_double_box_Vs_SFR

  FUNCTION chp_NLO_delta_2PE_double_box_Wt_SFR(w, L, D, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: D
    REAL(8) , INTENT(IN) :: delta
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -hA4*const(22)*(2.0D0*L + (4.0D0*delta*delta + w*w)*D)/(1296.0D0)
    
  END FUNCTION chp_NLO_delta_2PE_double_box_Wt_SFR

  FUNCTION chp_NLO_delta_2PE_double_box_Ws_SFR(q2, w, L, D, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: q2
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: D
    ...
    res = -q2*chp_NLO_delta_2PE_double_box_Wt_SFR(w, L, D, delta, impi)
    
  END FUNCTION chp_NLO_delta_2PE_double_box_Ws_SFR


  FUNCTION chp_NNLO_delta_2PE_triangle_Vc_SFR(w, L, S, D, delta, impi) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: S
    REAL(8) , INTENT(IN) :: D
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -hA2*const(22)*delta*( 6.0D0*S*(4.0D0*c1*mpi2(impi)-2.0D0*c2*delta*delta - c3*(2.0D0*delta*delta + S))*D + &
         (-24.0D0*c1*mpi2(impi) + c2*(w*w-6.0D0*S) + 6.0D0*c3*(2.0D0*delta*delta+S))*L)/(18.0D0)
    
  END FUNCTION chp_NNLO_delta_2PE_triangle_Vc_SFR

  FUNCTION chp_NNLO_delta_2PE_triangle_Wc_SFR(w, L, S, D, delta) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: S
    REAL(8) , INTENT(IN) :: D
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -hA*(b3+b8)*const(22)*delta*( 12.0*delta*delta*S*D + (-w*w + 6.0D0*S)*L )/(108.0D0)
    
  END FUNCTION chp_NNLO_delta_2PE_triangle_Wc_SFR

  FUNCTION chp_NNLO_delta_2PE_triangle_Wt_SFR(w, L, D, delta) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: D
    REAL(8) , INTENT(IN) :: delta
    ...
    REAL(8) :: res
    !const(22) used insteado of 21
    res = -c4*hA2*const(22)*delta*( (w*w-4.0D0*delta*delta)*D-2.0D0*L )/(72.0D0)
    
  END FUNCTION chp_NNLO_delta_2PE_triangle_Wt_SFR

  FUNCTION chp_NNLO_delta_2PE_triangle_Ws_SFR(q2, w, L, D, delta) RESULT(res)
    REAL(8) , INTENT(IN) :: q2
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: D
    ...
    res = -q2*chp_NNLO_delta_2PE_triangle_Wt_SFR(w, L, D, delta)

  END FUNCTION chp_NNLO_delta_2PE_triangle_Ws_SFR

  FUNCTION chp_NNLO_delta_2PE_single_box_Wc_SFR(w, L, S, D, delta) RESULT(res)
    REAL(8) , INTENT(IN) :: w
    REAL(8) , INTENT(IN) :: L
    REAL(8) , INTENT(IN) :: S
    REAL(8) , INTENT(IN) :: D
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -(b3+b8)*gA2*hA*delta*const(22)*( 6.0D0*S*S*D + (w*w-12.0D0*(delta*delta + S))*L )/(108.0D0) 
    
  END FUNCTION chp_NNLO_delta_2PE_single_box_Wc_SFR
  ...
  ! n      : cutoff order 
  ! LAMBDA is accessed from the chp constant chp_lambda
  FUNCTION freg_delta(pfinal, pinit, n) RESULT(res)
    ...
    exponent = (p2**n/lambda_delta**(2.d0*n) + &
         pp2**n/lambda_delta**(2.0D0*n) )
    
    res = dexp(-exponent)
    
  END FUNCTION freg_delta
  ...
  ! I4 = int_0^1 dxint I4int(xint)
   function I4int(q1,q3,z,impi) result(res)
     ...
     do j = 1,mesh_amount
        ...
        res = res + wpnt*( (q3**2 + q1**2*(-1.d0 + 2.d0*xint) + 2.d0*q1*q3*xint*z)/sqrt(mpion2 - q1**2*(-1.d0 + xint)*xint) &
             + (q1**2 + q3**2*(-1 + 2.d0*xint) + 2.d0*q1*q3*xint*z)/sqrt(mpion2 - q3**2*(-1.d0 + xint)*xint))&
             /(16.*PI*(4.d0*(mpion2 - q1**2*(-1.d0 + xint)*xint)*(q1**2 + q3**2 + 2.d0*q1*q3*z) +(q3**2 + q1**2*(-1.d0 + 2.d0*xint) + 2.d0*q1*q3*xint*z)**2.d0))
     end do
...
contains 
  ...
  complex*16 function chiral_pot(p,q,r,s)
    ...
    use chiral_constants
    use ang_mom_functions, only : tjs 
    use chiral_tables, only : delta_tab
    implicit none 
...
!!$       !! XXXXXXX  New Stuff XXXXXXX  !!!!
    ...
    ELSE
       vdir = (vlo + cont_lo + cont_nlo + vnlo + vnnlo + cont_n3lo + vn3lo) * &
  ...
  function chiral_NN_with_delta(p,q,r,s) result(res)
    USE single_particle_orbits
    USE constants
    use chiral_constants
    use ang_mom_functions, only : tjs 
   ! use chiral_tables, only : delta_tab
    ...
    REAL*8 :: loop_L
    REAL*8 :: loop_A
    real*8 :: loop_sigma, loop_D, loop_H
    real*8 :: sigma_dot_sigma,qxk2
    INTEGER :: imnuc_2PE
    ...
    if(delta_chiral_order > LO) then
       ...
       loop_sigma = 2.d0*mpi2(impi) + q2 - 2.d0*delta_split**2.d0
       ...
       IF (t1 + t2 == 0) THEN 
          ...
          WT = 0.d0 
          DO Tiso = 0, 2, 2
             cg1 = tjs(1,1,Tiso,t1,t2,0)*sqrt(Tiso+1.d0)
             cg2 = tjs(1,1,Tiso,t3,t4,0)*sqrt(Tiso+1.d0)
       ...
       Wc = Wc + chp_NLO_delta_2PE_single_triangle_Wc_SFR(loop_w, loop_L, loop_sigma, loop_D, delta_split)
       
       !delta leading single box diagrams
       Wc = Wc + chp_NLO_delta_2PE_single_box_Wc_SFR(q2,loop_L, loop_sigma, loop_D, delta_split, 2)
       Vs = Vs + chp_NLO_delta_2PE_single_box_Vs_SFR(q2, loop_w, loop_L, loop_D, delta_split, 2)
       VT = VT + chp_NLO_delta_2PE_single_box_VT_SFR( loop_w, loop_L, loop_D, delta_split, 2)
       Vc =      chp_NLO_delta_2PE_single_box_Vc_SFR(q2, loop_A, delta_split, 2)
       WT =      chp_NLO_delta_2PE_single_box_WT_SFR( loop_w, loop_A, delta_split, 2)
       Ws =      chp_NLO_delta_2PE_single_box_Ws_SFR(q2, loop_w, loop_A, delta_split, 2)
       
       !delta leading double box diagrams:
       Wc = Wc + chp_NLO_delta_2PE_double_box_Wc_SFR(    loop_W, loop_L, loop_sigma, loop_D, loop_H, delta_split, 2) 
       Vs = Vs + chp_NLO_delta_2PE_double_box_Vs_SFR(q2, loop_w, loop_L, loop_D, delta_split, 2)
       VT = Vt + chp_NLO_delta_2PE_double_box_VT_SFR( loop_w, loop_L, loop_D, delta_split, 2)
       Vc = Vc + chp_NLO_delta_2PE_double_box_Vc_SFR( loop_L, loop_sigma, loop_D, loop_H, delta_split, 2)
       WT = WT + chp_NLO_delta_2PE_double_box_WT_SFR( loop_w, loop_L, loop_D,  delta_split, 2)
       Ws = Ws + chp_NLO_delta_2PE_double_box_Ws_SFR(q2, loop_w, loop_L, loop_D, delta_split, 2)

       
       
       Wc = Wc*chp_tau_dot_tau_mtx(t1,t2,t3,t4)*delta(m1,m3)*delta(m2,m4)
       Vs = Vs*chp_sigma_dot_sigma_mtx(m1,m2,m3,m4)*delta(t1,t3)*delta(t2,t4)
       Vt = Vt*chp_sigma1_dot_q_sigma2_dot_q_mtx(m1,m2,m3,m4,qtrans)*delta(t1,t3)*delta(t2,t4) 
       Vc = Vc*delta(m1,m3)*delta(m2,m4)*delta(t1,t3)*delta(t2,t4)
       WT = WT*chp_sigma1_dot_q_sigma2_dot_q_mtx(m1,m2,m3,m4,qtrans)*chp_tau_dot_tau_mtx(t1,t2,t3,t4)
       WS = WS*chp_sigma_dot_sigma_mtx(m1,m2,m3,m4)*chp_tau_dot_tau_mtx(t1,t2,t3,t4)
       
       vnlo = (  Wc + Vs + Vt + Vc + WT + WS  ) 
    ...
    if(delta_chiral_order > NLO) then 
       ...
       Vc = Vc + chp_NNLO_delta_2PE_triangle_Vc_SFR(loop_w, loop_L, loop_sigma, loop_D, delta_split, 2) 
       WT = WT + chp_NNLO_delta_2PE_triangle_Wt_SFR(    loop_w, loop_L, loop_D , delta_split   )
       Ws = Ws + chp_NNLO_delta_2PE_triangle_Ws_SFR(q2, loop_w, loop_L, loop_D, delta_split)
       !Wc = chp_NNLO_delta_2PE_triangle_Wc_SFR( loop_w, loop_L, loop_sigma, loop_D, delta_split   )
    ...
    vdir = ( cont_lo  + vlo  + cont_nlo + vnlo  + vnnlo  ) * & 
         relativity_factor * freg_delta(prel, pprel, nexp_NN)
    ...
    res = hbarc**3 * (vdir)/volume 
     ...
     function chiral_3nf_with_delta(p,q,r,s,t,u) result(res)
       USE single_particle_orbits
       USE constants
       use chiral_constants
       use chiral_tables 
       ...
       INTEGER :: nx1, ny1, nz1, nx2, ny2, nz2, nx3, ny3, nz3, nx4, ny4, nz4, nx5, ny5, nz5 , nx6, ny6, nz6 
       REAL*8 :: k1(3), k2(3), k3(3), k4(3), k5(3), k6(3), q1xq2(3), q2xq3(3), q1xq3(3)
       real*8 :: kmean1(3), kmean2(3),  kmean3(3)
       REAL*8 :: qtrans1(3), qtrans2(3), qtrans3(3), prel(3), pprel(3)
       REAL*8 :: q22, q32, q12, c1_bar, c3_bar, c4_bar
       REAL*8 :: delta, nucleon_mass, coeff
       ...
       c1_bar = C1_3NF 
       c3_bar = C3_3NF - 4d0*hA2/(9d0*delta_split)
       c4_bar = C4_3NF + 2d0*hA2/(9d0*delta_split)
       ...
       v3nf_sum = v3nf_sum1 + v3nf_sum2 + v3nf_sum3

       
       res = freg_3nfnonlocal(k1,k2,k3)*freg_3nfnonlocal(k4,k5,k6)*hbarc**6*(v3nf_sum)/volume**2
       


     end function chiral_3nf_with_delta
     
  complex*16 function chiral_3nf_with_delta_asym(i,j,p,k,l,q)
    USE single_particle_orbits
    USE constants
    use chiral_constants
    ...
    chiral_3nf_with_delta_asym =  ( chiral_3nf_with_delta(i,j,p,k,l,q) - chiral_3nf_with_delta(i,j,p,l,k,q) &
         - chiral_3nf_with_delta(i,j,p,q,l,k) - chiral_3nf_with_delta(i,j,p,k,q,l) &
         + chiral_3nf_with_delta(i,j,p,l,q,k) + chiral_3nf_with_delta(i,j,p,q,k,l) )
    
  end function chiral_3nf_with_delta_asym
